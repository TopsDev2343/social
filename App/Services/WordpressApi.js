...
WordpressAPI.prototype.getNonceRegister = function () {
  const requestUrl = `${this.url}/api/get_nonce/?controller=user&method=register`;

  return this._requestPost(requestUrl).then(data => {
    return data;
  });
};
...
WordpressAPI.prototype.generateAuthCookie = async function ($email, $password) {
  ...
  if (typeof data.status !== 'undefined' && data.status == 'ok') {
    ...
    const requestUrl = `${this.url}/api/user/generate_auth_cookie/?insecure=cool&username=${$email}&password=${$password}`;
...
WordpressAPI.prototype.register = async function ($email, $password, $name) {
  ...
  if (typeof data.status !== 'undefined' && data.status == 'ok') {
    ...
    const requestUrl = `${this.url}/api/user/register/?nonce=${nonce}&email=${$email}&username=${$email}&display_name=${$name}&user_pass=${$password}&seconds=100`;
