...
const CardLayout = ({ viewPost, hideTitle, post }) => {
  ...
  const mediaDetailRatio =
    post &&
    has(post, 'better_featured_image') &&
    size(get(post, 'better_featured_image.media_details')) > 0
      ? get(post, 'better_featured_image.media_details.height') /
        get(post, 'better_featured_image.media_details.width')
      : 1;
  ...
  return (
    ...
    >
      <ImageCache
        source={{ uri: imageURL }}
        style={[css.imagePanelOne, dynamicHeight && { height }]}
      />
