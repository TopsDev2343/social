...
import { View, Image, Dimensions, Linking, WebBrowser } from 'react-native';
...
export default class Index extends PureComponent {
  ...
  render() {
    const htmlContent = Config.EnableSanitizeHtml
      ? sanitizeHtml(this.props.html, {
    ...
    const renderers = {
      ...
      iframe: (htmlAttribs, children, convertedCSSStyles, passProps) => {
        if (htmlAttribs.src) {
          ...
          const newHeight =
            height > 0 ? (height * newWidth) / width : width * 0.7;
    ...
    return (
      <View style={{ padding: 12 }}>
        <HTML
          html={
            Constants.RTL
              ? `<div style="text-align: left;">${htmlContent}</div>`
              : htmlContent
          }
