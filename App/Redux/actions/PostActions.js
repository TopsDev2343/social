...
export const fetchStickyPost = () => {
  ...
  const api = wp.posts().sticky(sticky).tags(tag).categories(category);
...
export const fetchPhotos = page => {
  const limit = Constants.PagingLimit;
  const api = wp.posts().perPage(limit).page(page);
...
export const fetchPostsRelated = (page, categoryId, postCurrent) => {
  const api = wp
    ...
    .page(page);
  return dispatch =>
    fetch(dispatch, api, POST_RELATED_FETCH_SUCCESS, {
...
export const fetchPostById = id => {
  let api = wp.posts().id(id).sticky(false);
